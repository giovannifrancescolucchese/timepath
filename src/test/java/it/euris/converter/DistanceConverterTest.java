...
class DistanceConverterTest {
    ...
    @ParameterizedTest(name="Given {0} Km Then return {1} nautical miles ")
    @CsvSource({
            "0.0,  0.0",
            "1.852, 1.0",
            "1852, 1000",
    ...
    void convertKilometersToNauticalMiles(Double kilometers, Double expectedResult) throws DistanceConverterException {
        //arrange
        dc.setFromKilometers(kilometers);
        //act
        Double result=dc.getNauticalMiles();
        //assert
        assertEquals(expectedResult, result, ()->String.format("%1$,.2f km  should equals to %2$,.2f nautical miles", kilometers,expectedResult));
    }
